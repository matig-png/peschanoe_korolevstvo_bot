...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    user_role = "Администратор" if message.from_user.id == OWNER_ID else "Пользователь"
    await message.reply(f"✨ <b>Добро пожаловать в систему Искр!</b>\n\nВы зашли как: {user_role}\n\nКоманда для перевода:\n<code>перевести @ник сумма</code>", 
...
async def call_bal(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(
        f"💳 <b>Ваши счета:</b>\n\n"
        f"✨ Искры: <code>{s_str}</code>\n"
        f"🌗 Луны: <code>{m_str}</code>\n\n"
...
async def fast_transfer(message: types.Message):
    ...
    t_name = escape(f"@{t_user['username']}") if t_user and t_user['username'] else f"ID:{target_id}"
    await message.reply(f"✅ Успешно переведено <code>{amount}</code> ✨ пользователю {t_name}", parse_mode="HTML")
    
    try:
        await message.bot.send_message(target_id, f"✨ Вам пришло <code>{amount}</code> искр от {escape(message.from_user.full_name)}")
    except: pass
...
async def call_conv(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text("💱 <b>Обмен валют</b>\n\nКурс: 1 🌗 = 2 ✨", reply_markup=kb.as_markup(), parse_mode="HTML")
...
async def call_adm(callback: types.CallbackQuery):
    ...
    kb.row(InlineKeyboardButton(text="◀️ Назад", callback_data="back_main"))
    await callback.message.edit_text("⚙️ <b>Админ-панель владельца</b>", reply_markup=kb.as_markup(), parse_mode="HTML")
...
async def adm_act(callback: types.CallbackQuery, state: FSMContext):
    ...
    p = "💰 Введите <code>@ник Сумма</code>:" if act == "give" else "❄️ Введите <code>@ник</code> пользователя:"
    await callback.message.edit_text(p, reply_markup=cancel_kb(), parse_mode="HTML")
    ...
    try:
        if d['act'] == "give":
            ...
            if u_id:
                ...
                await message.answer(f"✅ Выдано {a} ✨ пользователю <code>{u_id}</code>", parse_mode="HTML")
        else:
            u_id = db.find_user(message.text)
            if u_id:
                st = (d['act'] == "frz")
                db.set_bot_data(u_id, is_frozen=st)
                await message.answer(f"✅ Статус <code>{u_id}</code> изменен.", parse_mode="HTML")
